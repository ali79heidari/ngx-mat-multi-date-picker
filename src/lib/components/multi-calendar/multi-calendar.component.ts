import { Component, Input, Output, EventEmitter, Inject, OnChanges, SimpleChanges, ViewEncapsulation, Optional } from '@angular/core';
...
@Component({
  ...
  template: `
    <mat-card class="multi-calendar-card">
        <mat-calendar 
            [selected]="selected"
...
export class MultiCalendarComponent implements OnChanges {
  ...
  // Outputs
  @Output() selectedChange = new EventEmitter<Dayjs | null>();
  @Output() monthSelected = new EventEmitter<Dayjs>();
  @Output() yearSelected = new EventEmitter<Dayjs>();

  constructor(@Optional() private _adapter: DateAdapter<Dayjs>) {}
  ...
  onSelectedChange(date: Dayjs | null) {
      this.selected = date;
